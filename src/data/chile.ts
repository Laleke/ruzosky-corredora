/** Catálogo oficial de regiones y comunas de Chile (16 regiones). */
export const REGIONES: { nombre: string; comunas: string[] }[] = [
  {
  ...
  },
  {
    ...
    comunas: [
      ...
      "Chanco", "Pelluhue", "Curicó", "Hualañé", "Licantén", "Molina", "Rauco",
...
export function comunasDeRegion(region: string): string[] {
  const r = REGIONES.find(
    (x) => x.nombre.toLowerCase() === region.trim().toLowerCase()
  );
  return r?.comunas ?? [];
}
