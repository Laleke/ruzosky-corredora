...
**Última sesión: 2026-07-27.** App **en producción** (Vercel), redeploy con cada push. Repo `github.com/Laleke/ruzosky-corredora`. Rama `main` **pusheada hasta `db46805`** — todos los commits de esta sesión (5) ya están en `origin/main`, con autorización explícita de Eduardo en cada push. `tsc --noEmit`, `next build` y `npm test` (12/12) quedaron verdes después de cada entregable de la sesión; no hay cambios sin commitear.

### ✅ Migraciones 0017/0018/0019 ejecutadas — 0020 pendiente

Eduardo confirmó que corrió las 3 migraciones de política RLS `DELETE` (`propiedades`, `arrendatarios`, `propietarios`) en el SQL Editor de Supabase — el botón "Eliminar" en esos 3 módulos ya debería funcionar en producción. **Falta ejecutar `supabase/migrations/0020_contratos_delete.sql`** (misma política, para Contratos) — sin ella, "Eliminar" en Contratos no funcionará (afecta 0 filas en silencio, mismo gotcha de siempre).

### Resumen de lo hecho esta sesión (5 commits, en orden)
...
Se abrió una nueva etapa de la app (uso con datos reales, no solo demo): tema oscuro grafito + burdeo (antes blanco), navegación reducida a los 4 módulos en uso (Propiedades, Propietarios, Arrendatarios, Contratos — el resto de módulos sigue en el código, solo oculto del menú), y un **patrón de página unificado** documentado en **`docs/DISENO_PAGINAS.md`** (léelo primero si vas a tocar cualquier página de listado/detalle/creación):
...
2. Seguir probando en el celular y dar feedback de ajustes — patrón ya validado en Propiedades/Arrendatarios/Propietarios; **Contratos y el Dashboard rediseñado (esta sesión) aún no se han probado a fondo en el celular** — revisar tarjetas, filtros, detalle, wizard, asignar/quitar arrendatario, eliminar, y las 7 tarjetas KPI del Dashboard.
...
7. Deuda de bajo riesgo anotada esta sesión: Propiedad en el detalle/wizard de Contratos usa `<select>` nativo en vez de un combobox con búsqueda (el `Combobox` genérico del proyecto no soporta id≠label; ahora que existe `select-combo.tsx` — creado para Tipo de cuenta — se podría reusar ahí si se vuelve a pedir) — revisar si la lista de propiedades crece mucho.
