...
**Última sesión: 2026-07-07.** App **en producción** (Vercel), redeploy con cada push. Repo `github.com/Laleke/ruzosky-corredora`. Login real: `admin@ruzosky.cl`. Rama `main` **empujada hasta `7df2ff0`** (esta sesión — `6c4c0b8`…`7df2ff0` — ya está pusheada; Eduardo autorizó el push explícitamente). Vercel debería haber redesplegado automáticamente.

**Estado del producto:**
- MVP + **Liquidaciones** + **Centro Documental** + **Gastos** + **Reportes Financieros** + **Dashboard Operativo** (tareas pendientes), todos desplegados/implementados y validados por QA.
...
- Módulos: Propietarios, Propiedades (+copropiedad N:M), Arrendatarios, Contratos, Cobros (+filtros +indicador pendientes), Liquidaciones (+guardia de duplicados +indicador pendientes), **Documentos** (Storage privado + signed URLs + versiones), **Gastos** (fuente oficial), **Reportes** (KPIs + gráficos SVG propios + export CSV/Excel/PDF), **Dashboard Operativo** (tareas pendientes).
- **Vitest** montado (`npm test`, 12 pruebas verdes) para lógica pura (filtros, etiquetas).

**Lo que se hizo en esta sesión (2026-07-07):**
1. **QA de simplificación de formularios (matriz Q1–Q7) aprobada** ✅ — sin hallazgos que bloqueen.
2. **Ronda de mejoras UX/consistencia** (8 commits, ver detalle en `## Últimos Cambios` 2026-07-07): limpieza de Gastos (filtro/detalle/ayuda), **fix de bug real en Documentos** (propietario no se derivaba automáticamente), filtros en Cargos, guardia de liquidación duplicada, fusión de bloques Descuentos/Gastos en Liquidaciones, indicadores de pendientes en Liquidaciones y Cobros, y el nuevo **Dashboard Operativo**.
3. Ninguno de estos cambios requirió migración de BD. Build verde, `tsc` limpio, 12 tests en cada commit.
4. **Push autorizado y ejecutado** (`0ca2e5d..7df2ff0`) — los 8 commits ya están en `origin/main`.
5. **QA6 entregada** (matriz de prueba para esta ronda) como artifact: https://claude.ai/code/artifact/bf888b5a-c9bb-4e6d-a9ee-73416c5505da — cubre los 8 puntos + 4 checks de regresión. **Pendiente que Eduardo la ejecute y confirme resultado.**

**Pendiente / próximo (en orden):**
1. **Ejecutar QA6** (link arriba) y volcar resultado (OK/FALLA por ítem) en esta sección al cerrar.
2. **Roadmap de Hardening — Sprint 1 aún incompleto**: faltan **T2** (auditoría en pagos/cargos/contratos/catastro), **T3** (backups + prueba de restauración), **T5** (gate de rol en `DashboardLayout`), **T11** (regenerar `database.types.ts` con `supabase gen types`). *(T1/T1b hechos.)*
3. **Fase C — R3 gastos compartidos** (Backlog QA): dividir un gasto propietario/arrendatario por **porcentaje** (ya sin "Responsable"); el gasto compartido genera un **Cobro al Arrendatario** vinculado. **Requiere migración** (`+compartido` en uso, `porcentaje_propietario`, `cargos.gasto_id`). Ver reglas R3/R5 en `## Backlog QA 1`.
4. **Reglas R1/R2/R5 pendientes** (Backlog QA, motor de liquidación): R1 liquidaciones cerradas + "pendiente de liquidar" + arrastre; R2 sin negativas + saldo por **propietario×propiedad**; R5 gastos en cuotas / movimientos programados. Requieren migración y tocan `calcularLiquidacion`; hacer **T16 (tests)** antes.
5. **Aislamiento cross-tenant** (seguridad H2/T1b): validado por inspección de RLS, **falta prueba funcional con 2ª empresa** (se hará al onboardear 2º tenant / T20).
6. **Dashboard Operativo — evolución**: definir regla de negocio de "documento obligatorio por tipo de propiedad" para habilitar el indicador de documentos sin respaldo (hoy placeholder "Próximamente").
7. Menores: reinstalar PWA para tomar ícono/nombre; renombrar `empresas.nombre`.
