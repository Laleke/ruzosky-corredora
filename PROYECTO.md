...
- **Select de "Tipo de cuenta" se veía distinto a los combobox**: primer intento (`select-styled.tsx`, un `<select>` nativo con chevron superpuesto) no fue suficiente — en el celular el select nativo sigue abriendo el picker propio del sistema operativo, visualmente distinto sin importar el CSS. Eduardo pidió tomar Región/Comuna como referencia real. Se creó `src/components/select-combo.tsx`: mismo comportamiento que `Combobox` (input propio + lista desplegable propia del sistema, sin picker nativo) pero soporta pares `{value, label}` en vez de un string único — necesario porque "Cuenta corriente" (label) ≠ "corriente" (value guardado en la BD, enum de Postgres). Aplicado a Tipo de cuenta en detalle y wizard de Propietarios. `select-styled.tsx` queda solo para Tipo de persona (2 opciones fijas, sin el mismo reclamo).
