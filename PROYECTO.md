...
App **RZK Prop** operativa: auth, dashboard con KPIs (simplificado a 3: Propiedades vs arrendadas, Deuda pendiente, Cargos morosos) + **Dashboard Operativo (tareas pendientes)**, y módulos **Propietarios, Propiedades (+copropiedad N:M), Arrendatarios, Contratos (+ reajuste de canon vía UF con revisión manual), Cobros (cargos+pagos), Liquidaciones (con descuento automático de gastos), Documentos (Storage privado + versiones), Gastos (fuente oficial) y Reportes Financieros**. **Menú completo activado** (2026-07-29): los 5 módulos que estaban ocultos (Cobros, Liquidaciones, Gastos, Documentos, Reportes) ya aparecen en el sidebar y **ya tienen el mismo patrón de diseño que Propiedades/Propietarios/Arrendatarios/Contratos** (tarjetas `ui.listCard`, filtro colapsable, wizard de creación). **23 migraciones (`0001`–`0023`) aplicadas y verificadas en Supabase.** Build de producción verde, `tsc` limpio, **Vitest** con 12 pruebas. **PWA con banner de actualización** ("Hay una versión nueva — Actualizar", vía `SerwistProvider`) para que un deploy nuevo no quede invisible en pestañas ya abiertas.
**UI: patrón de página unificado en los 9 módulos** (listado en tarjetas, detalle con edición en línea, wizard de creación, filtro colapsable). Fuente de verdad: `docs/DISENO_PAGINAS.md` + **`docs/PAGINAS_NUEVAS.md`** (checklist rápido de 6 puntos para no dejar piezas afuera al migrar/crear una página — nació porque la primera pasada de Cobros/Liquidaciones/Gastos/Documentos/Reportes aplicó el wizard y el filtro pero se quedaron afuera las tarjetas, el combobox con buscador y la posición del mensaje de cancelar; ya corregido).
**Filosofía vigente: app centrada en la Propiedad** — el usuario solo elige Propiedad + datos; propietario/contrato/arrendatario se derivan automáticamente (ver "Simplificación de flujo"). **Roadmap oficial de Hardening** y **Backlog QA** (reglas R1–R5) siguen **sin avance** — todo el foco de las últimas sesiones fue UI/UX (rediseño completo + reajuste de canon).
...
**Última sesión: 2026-07-29.** App **en producción** (Vercel), redeploy con cada push. Repo `github.com/Laleke/ruzosky-corredora`. Rama `main` **pusheada hasta `bb5c404`** (4 commits de esta sesión, autorizados explícitamente por Eduardo). Sin cambios sin commitear al cerrar. `tsc --noEmit` y `next build` verdes tras cada entregable.

### ✅ Migraciones 0021/0022/0023 ejecutadas (Eduardo confirmó, incluyendo un falso intento inicial contra el proyecto Supabase equivocado — ya resuelto)

`canon_actual`, `canon_uf_base` y `fecha_proximo_reajuste` ya existen en `contratos` en producción. Migración `0020_contratos_delete.sql` (pendiente de sesiones anteriores) — **confirmar si ya se ejecutó**, no quedó registrado en esta sesión que Eduardo la corriera.

### Corrección de formato ya commiteada y pusheada (`bb5c404`)

Corrección de formato tras feedback de Eduardo ("no aplicaste a las tarjetas... los combobox deben tener otro formato... botón cancelar debe ponerse el mensaje encima de pantalla"): tarjetas `ui.listCard` en Cobros/Liquidaciones/Gastos/Documentos (antes `<table>`), `ComboboxOpcion` nuevo (`src/components/combobox-opcion.tsx`) en los pasos "Propiedad"/"Propietario" de los 4 wizards nuevos, mensaje de "Cancelar" reposicionado a overlay fijo arriba de pantalla, `AccionesArchivo` con variante `oscuro`, y `docs/PAGINAS_NUEVAS.md` (checklist nuevo). Build y `tsc` verdes. **Pendiente real**: que Eduardo lo pruebe visualmente — no se pudo verificar en navegador esta sesión (sin Playwright/chromium-cli disponible en el entorno; instalarlo quedó descartado por costo/tiempo, no intentado a fondo).

### Resumen de lo hecho esta sesión anterior 2026-07-27 (5 commits, en orden)
...
1. **Eduardo prueba visualmente en el celular** el menú completo y los 5 módulos recién migrados (Cobros, Liquidaciones, Gastos, Documentos, Reportes) — tarjetas, filtro colapsable, wizard de creación, combobox de Propiedad/Propietario, overlay de Cancelar. No se verificó en navegador esta sesión.
2. **Confirmar si `0020_contratos_delete.sql` ya se ejecutó** (quedó pendiente de una sesión anterior, sin confirmación explícita desde entonces) — sin ella "Eliminar" en Contratos afecta 0 filas en silencio.
3. Probar el flujo de reajuste de canon con datos reales: en Vicuña Mackenna, esperar al 01-09-2026 (o forzar antes) y usar "Aplicar reajuste (UF)" para confirmar que el resultado sigue calzando con la fórmula validada (`canon_uf_base × UF del corte`).
4. **Deuda anotada esta sesión**: los 4 wizards originales (Propiedades/Arrendatarios/Propietarios/Contratos) siguen con el mensaje de "Cancelar" inline (viejo patrón) — si se vuelve a tocar alguno, aplicar el overlay fijo arriba de pantalla para que los 8 wizards queden consistentes (ver `docs/PAGINAS_NUEVAS.md`, punto 3).
5. Una vez estable, correr `limpiar_datos_prueba` + `cargar_datos_reales_803_1907A` para empezar a usar la app con datos reales.
6. Roadmap de Hardening — Sprint 1 sigue incompleto: faltan T2 (auditoría), T3 (backups), T5 (gate de rol), T11 (regenerar `database.types.ts`). No se tocó en las últimas sesiones (todo el foco fue UI).
7. Backlog QA (R1/R2/R3/R5, motor de liquidación) — sin cambios, sigue pendiente.
...
## Últimos Cambios
- 2026-07-29 (3 commits pusheados: `ae74a4a`→`85c0f0a`→`fb5afbc`, + trabajo de formato pendiente de commitear al cierre de la sesión) — **Botón atrás genérico, reajuste de canon vía UF con revisión manual, y menú completo con formato replicado.**
