...
- Solo tiene sentido si el registro no tiene relaciones que lo bloqueen. En vez de duplicar esa validación en la capa de aplicación, se **aprovecha la restricción de la base de datos** (`on delete restrict` en `contratos.propiedad_id` y `gastos.propiedad_id` — si hay contratos, por transitividad tampoco puede haber arrendatarios ni cargos/deudas sin contrato) y solo se traduce el error de violación de FK a un mensaje legible (`eliminarPropiedad` en `actions.ts`).
- UI: botón secundario en tono rojo apagado (`bg-red-950/40 text-red-200`), al final de la pantalla de detalle, con `confirm()` nativo antes de ejecutar (acción destructiva irreversible). No es parte del `<form>` de edición — es una acción independiente.
